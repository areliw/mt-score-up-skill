...
- **`deep-research` demote `semi-stable` → `draft` + no-tool guard:** round 6 พบว่าเป็น *tool-execution* skill — โมเดลที่ไม่มี search/fetch tool โหลดแล้ว **fabricate citation table** (regression −0.33). เพิ่ม guard หัวไฟล์ (ไม่มี tool → ห้ามสร้างตาราง/ลิงก์เอง ส่งแผนการค้นแทน); re-verify แล้ว regression ปิด (rescued·tie·tie, +0.67, 0 regression). re-promote ทีหลังกับ codex.
- **Promote round 6 → `semi-stable` (codex-gated):** `report-up-judgment` · `writing-judgment` flip `draft → semi-stable` (A/B lift + codex OK). อีก 5 ตัวที่ A/B lift ผ่านแต่ **codex BLOCK** (ยก heuristic เป็นกฎสากล / overstate) คง `draft` รอ hedging-fix + re-codex: `interprofessional-communication-judgment` · `humanize-ai-writing` · `ab-test-judgment` · `lead-intelligence-judgment` · `lab-clinic-business-judgment`.
