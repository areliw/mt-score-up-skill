...
```
mt-score-up-skill/
├── skills/        # ★ 94 สกิล — copy (แช่แข็ง) หรือโหลดสดผ่าน INDEX.md (auto-sync)
...
- **ทดสอบครบทั้ง 94 สกิลด้วย A/B blind-judge** (control + ทำซ้ำ 3–5 รอบ) — **32 ตัวที่ยกคะแนนได้จริง (Δ ผ่านเกณฑ์) + codex review + หลักฐานยังตรงเนื้อปัจจุบัน = สถานะ `semi-stable`** (ตัวที่ผลลบ/ไม่ชัดถูกกั้นไว้ที่ `draft` — ดูบรรทัดถัดไป)
- เกณฑ์ตัดสิน **สเกลตาม noise ของรอบนั้น (Δ ≥ 2·SE)** ไม่ใช่เลขตายตัว — กันหลงเชื่อ lift ที่จริงเป็นแค่ความผันผวน
- **ไม่ promote สกิลที่ A/B ยังไม่บวกชัด** — ตัวที่ผลลบ/ไม่ชัดถูกกั้นไว้ที่ `draft` (ผลลบรอบเดียวมักเป็น noise: เคยเจอ −3 กลายเป็น +1.3 ตอนเทสต์ซ้ำ) → **ไม่มีสกิล `semi-stable` ตัวไหนที่ A/B ติดลบ**
- แก้เนื้อสกิลหลังเทสต์ = หลักฐาน stale อัตโนมัติ (**hash-currency gate** ใน CI) → คะแนนบนกระดานไม่ค้างของเก่า
