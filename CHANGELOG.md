...
- **Backfire-fix sweep — 10 `draft` skills codex-improved for over-triggering (all stay `draft`)**: every skill that A/B-*backfired* (weak model scored worse WITH it) was diagnosed by codex and fixed. **One mechanism across all 10:** the skill made a weak model **dump the whole framework/checklist and bury the actual answer on a simple question** (ceremony, not bad content). Fix everywhere = an **"ตอบคำถามจริงก่อน — ใช้กรอบเมื่อช่วยจริง" (answer-first) carve-out** + an over-trigger anti-pattern.
  - Non-clinical (6, codex EDIT + take/drop): `offload-to-automation` (over-trigger + no-tool code-dump → answer-first), `ai-agent-team` (opened with org-chart/routing ceremony), `digital-judgment` (**also fixed a real PDPA legal error** — consent is one lawful basis, not the only one — + over-refuse), `never-lose-a-file` (dumped full doctrine; kept load-bearing delete-safety warnings), `know-yourself` (forced multi-round interview ritual; ⛔ scoped to master-profile), `lab-clinic-business-judgment` (dumped all 6 forks; softened absolutes).
  - Clinical 🩸 (4, **flag-only** — codex EDIT but ZERO clinical content changed, only additive answer-first carve-out + `last_edited`; every safety warning kept): `clinchem-judgment`, `toxicology-judgment`, `infection-control-judgment`, `bloodbank-judgment`. codex raised **1 CLINICAL-FLAG** on `infection-control` Fork 4 (draining-wound/incontinence grouped under negative-pressure logic) — **since fixed in #111** (reclassified as Contact; AIIR reserved for airborne agents).
  - **Verification = codex expert-review (per `eval/IMPROVE-PLAYBOOK.md`: single-edit A/B is noise-dominated) + a full re-A/B refresh** (fresh over-trigger traps) so no stale backfire row remains. **Result: 0 of 10 still backfire** — every with-arm is now *safe*. Rescued: `infection-control` −1→**+2.33** (σ3.49). Lift: `ai-agent-team` +1.33, `toxicology` +1.0, `bloodbank` +1.0. Tie: `know-yourself` +1.0, `offload` +0.4, `clinchem` +0.4, `lab-clinic-business` +0.33. Style-cost (negative but safe, within/near noise): `digital` −0.6, `never-lose-a-file` −1.4. **`never-lose-a-file`** is the one that stayed negative: its first re-A/B *regressed* (with-arm 2.33 unsafe) → sharpened the carve-out (forbid ask-back, demand a concrete default) → re-A/B ×5 lifted with-arm to 3.0 **safe** but still −1.4 style-cost — because file-organization is **commodity knowledge the base already aces (4.4)**, an easy-scenario ceiling not editable further (per the playbook). `_ab_slim` refreshed for all 10; `ab-coverage.json` rebuilt.
...
### Added
- **`research-section-drafting` — new skill (สกิลที่ 95): ร่าง 5 ส่วนงานวิจัยจากไอเดีย → `semi-stable`**: DO-type drafting step that complements `manuscript-judgment`'s reviewer-lens ADVISE — fills the "generate the draft" gap in the R2R pipeline (proposal → design → stats → **draft** → manuscript). Core judgment = **fabrication firewall** (🟢 draftable-from-idea: title/intro/lit-review/methods-plan · 🟡 skeleton+placeholder only: results/discussion/abstract · 🔴 AI must never invent: result numbers/p-values, citations/DOI, EC numbers, PII) + Thai format map (ว.19 5-ส่วน ↔ thesis 5 บท ↔ journal IMRaD) + reporting-guideline pick (STROBE/STARD/CONSORT/PRISMA/CARE/SQUIRE). Sourced from a ว.19 senior-professional research-format guide + real Thai cryoprecipitate FVIII/fibrinogen papers as the worked example. **codex-reviewed** (5 MUST-FIX applied: no-source vs source-grounded citation modes, methods plan-vs-specific-values split, EC self-declaration ban, proposal=future/manuscript=past tense, format-mapping hedged as default-not-rule). **A/B ×5 Δ +0.8 (σ4, se0.2, without 4.2 → with 5.0, tier=full)** — honest read: base (Haiku) already refuses the headline fabrication trap, so the measured lift is on the senior nuances it kept missing ("แก้ทีหลัง"=still-fabrication, EC-before-data-collection, DOI-verify, the exact draftable-vs-wait line).
