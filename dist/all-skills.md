...
### 6. ถ้ามี footage แล้ว: ผ่าน content-retention gate ก่อนสั่งตัด
ใช้ fork นี้เฉพาะ **คลิปยาวที่ขายเรื่อง/ชาเลนจ์/การเปลี่ยนแปลง** เช่น gameplay, vlog, case journey — ไม่ใช่ทุกวิดีโอ:
1. ระบุ **promise** ที่คนดูรอฟัง/รอดูตั้งแต่ต้นได้ 1 ประโยค.
2. หา **movement** ที่ข้อมูล การตัดสินใจ อุปสรรค หรือสถานะเปลี่ยนจน payoff มีความหมาย; ไม่มีจำนวน beat ตายตัว และคลิป one-attempt/reveal อาจครบได้ด้วยจุดเปลี่ยนเดียว.
3. ยืนยัน **payoff** ที่ตอบ promise ได้จาก footage จริง — สำเร็จ, ล้มเหลว หรือผลลัพธ์ที่ซื่อสัตย์ก็ได้.

- ครบ 3 ข้อ → ทำ EDL ที่รักษา **เหตุและผลตามจริง** แล้วค่อยปรับจังหวะ เสียง และเอฟเฟกต์; เปิดด้วย preview/cold open ได้ถ้าระบุชัดและไม่บิดผลลัพธ์.
- ไม่มี promise ที่ footage รองรับ → หา moment เดียวทำ Short/Discovery หรือเปลี่ยน promise; ห้ามตั้งโจทย์ใหญ่ย้อนหลัง.
- มี movement แต่ไม่มี payoff → ลด promise ให้จบตรงหลักฐานที่มี, จบแบบค้างอย่างซื่อสัตย์เมื่อ format รองรับ หรืออัดเพิ่ม.
- มี payoff แต่ไม่มี movement ที่คุ้มเวลา → ทำ Short/reveal; ทำ Long เฉพาะเมื่อ process/analysis เป็นคุณค่าที่สัญญาไว้.
- ถ้าเป้าหมายแรกพังกลางทางแต่เกิดเป้าหมายใหม่จริง → เก็บเป็น Act 2 และบอกการ pivot ตรง ๆ ห้ามตัดให้เหมือนเป้าแรกยังสำเร็จ.
- **ไม่ยิงเกิน:** tutorial ใช้ `ปัญหา → ขั้นตอน → ผลลัพธ์`; cozy/ASMR/ambient ที่ promise คือบรรยากาศ ใช้ความต่อเนื่องของประสบการณ์แทนเดิมพันสะสม.
...
- **ขัด footage ที่ไม่มีเรื่องด้วย compression/effects** → เทคนิคทำให้เร็วหรือสวยขึ้นได้ แต่สร้าง progression/payoff ที่ไม่เคยเกิดไม่ได้; ต้องย้อนกลับไปเปลี่ยน format/promise หรืออัดเพิ่ม.
