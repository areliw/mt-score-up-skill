...
---
*เอกสารสเปก/ทิศทางสำหรับ maintainer — ไม่ใช่ instruction ให้ AI โหลดเป็น skill. สกิลคลินิกยังเป็น `status: draft` (ยังไม่ผ่าน clinical peer-review); สกิลกลุ่ม data/code (non-clinical) = `status: stable`.*
