...
- ✅ **hash-currency (item 1):** `eval/ab-coverage.json` (registry: status + **body_hash** = sha256 ของเนื้อหลัง frontmatter + ab/method/delta/screen/peer) สร้างด้วย `scripts/build_ab_coverage.py`. `check_maturity_gate.py` เทียบ body_hash ปัจจุบัน vs registry → semi-stable/stable ที่ **แก้ judgment หลัง A/B** = STALE → CI แดงจนกว่าจะ re-A/B + rebuild. hash เฉพาะ body จึงแก้ `status:`/`last_edited:` ไม่ trip (metadata churn ≠ judgment churn). verified: current→OK · body-edit→STALE · frontmatter-edit→OK. baseline = forward-looking reset stamp 2026-06-18 (ไม่ได้อ้างว่า A/B legacy ใช้ body นี้เป๊ะ — แค่ "จากนี้ไป แก้ body = ต้อง re-test").
- ⏳ **TODO ที่เหลือ:** item 4 (CHANGELOG/README/spec สะท้อน 10 ชั้นเต็ม) · L6 replicate (แกนที่ 2) · L7 peer-review จริง (ยังไม่มีตัวถึง stable).
