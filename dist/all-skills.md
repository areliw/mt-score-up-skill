...
- **ขายขึ้น "ผู้บริหาร รพ." (เหนือหัวหน้าแล็บ) = HEOR:** ผูกผลแล็บกับ outcome ที่ผู้บริหารสน — **ลด length-of-stay/อัตราครองเตียง · ลดค่าใช้จ่ายรวม · เพิ่ม throughput** (ไม่ใช่ CV/SD ที่ผู้บริหารไม่อิน). ⚠️ **insight ที่มักเข้าใจผิด (เฉพาะ รพ.ที่เตียงตึง + รายได้ front-loaded):** ครองเตียงลด ≠ รายได้หายเสมอ — ถ้ารายได้กระจุกวันแรก (ตรวจ/แอดมิต) + มีคิวรอเตียง → test ที่ทำคนไข้กลับเร็ว = **เพิ่ม margin/หมุนเตียง** (เถียงเป็น performance ได้). ⚠️ **แต่ไม่ใช่ทุกที่:** วันท้ายๆ อาจยังบิลยา/แล็บ/หัตถการ/ค่าแพทย์ · รพ.ที่เตียงว่าง/จ่ายแบบ FFS-per-diem อาจรายได้ลด · DRG/เหมาจ่าย/capitation = เป็น cost-avoidance/margin ไม่ใช่ revenue → **ขึ้นกับ payer mix + occupancy → ทำ business case จากข้อมูล รพ.นั้นจริง ไม่ใช่เลขลอย** (โยง `report-up-judgment` แปลเทคนิค→ผลกระทบ)
