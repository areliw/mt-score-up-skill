...
def compute(rows):
    ...
    for r in rows:
        ...
        items.append({
            ...
            # with/without are shown only for measured rows; synthetic rows reconstructed them
            # from delta (round6-probe backfill), so the leaderboard hides the pair for them.
            "with": None if synthetic else r.get("withScore"),
            "without": None if synthetic else r.get("withoutScore"),
            "synthetic": synthetic,
...
<div id="list"></div>
<div class="foot">ที่มา: eval/_ab_slim.json · regenerated by scripts/build_insights.py · benchmark ภายใน (Haiku proxy) — ทิศของสัญญาณ ไม่ใช่สถิติประชากร MT · Δ ติดลบของคลินิก = heuristic เหมารวม ไม่ได้แปลว่า AI ปลอดภัยเรื่องนั้น · บางแถว (round6-probe backfill) มี Δ วัดจริงแต่สกอร์ absolute ประเมินจาก Δ — จึงไม่แสดงคู่ without→with</div>
</div>
<script>
...
function render(){const list=document.getElementById("list");list.innerHTML="";
 ...
 f.forEach((r,i)=>{const[tl,tc]=tcls(r.tier);const cb=CATB[r.cat]||CATB.other;const pos=r.delta>=0;const mag=Math.min(Math.abs(r.delta)/4,1)*48;
  ...
  list.insertAdjacentHTML("beforeend",`<div class="row"><div class="rank">${i+1}</div><div class="nm">${r.label}<span class="catb" style="background:${cb[1]};color:${cb[2]}">${cb[0]}</span><br><small>${r.slug} · ${r.synthetic?'Δ วัดจริง · สกอร์ประเมินจาก Δ':r.without+'→'+r.with}</small></div>${bar}<div><span class="delta" style="color:${pos?'#0f6e56':'#a32d2d'}">${pos?'+':''}${r.delta.toFixed(1)}</span><br><span class="tier ${tc}">${tl}</span></div></div>`);});
