...
โค้ด/สคริปต์พังแล้วงงว่า "ผิดตรงไหน" → skill นี้ช่วย **วางลำดับไล่บั๊กให้มีวินัย** เพื่อเจอ *สาเหตุจริง* ไม่ใช่เดาแล้วแปะแก้มั่วจนพังเพิ่ม
...
- **ซัก/วางแผนจน analysis paralysis** — grill เพื่อความมั่นใจ 100% แล้วไม่เริ่มสักที; พอได้ verdict READY (หรือรู้ตัวแปรเดียวที่ยังขาด) ต้องลงมือ — แผนคือ hypothesis ปรับได้ตอนเจอของจริง ไม่ใช่สัญญาที่ต้องเป๊ะก่อนเริ่ม
...
- **ข้ามขั้น "หลุดมาได้ยังไง"** — ไม่ถามว่าทำไม defense (QC/check/LIS) ไม่จับ → ช่องโหว่เดิมเปิดอยู่
- **เชื่อว่า "QC ผ่าน = ปลอดภัยทุกชั้น"** — QC คุม analytical precision ไม่จับ pre/post (sample swap, ลงค่าผิด, แจ้ง critical ช้า)
...
- **งานที่ไหลต่อเนื่องห้ามขัด** — ดีบักจนใกล้เจอ root cause, เขียนจนไอเดียพุ่ง, อ่านเคสที่กำลังต่อภาพ → **อย่าหยุดตามนาฬิกา** ปล่อยให้ไหลจบ flow ค่อยพัก (จับเวลา = ทำลายของดี). ถ้าหมดนาฬิกาแต่ยังไหลอยู่ → ต่อให้จบความคิดก่อน อย่าตัดกลางประโยค
...
- **Primary = งานต้นฉบับ** (ทำการทดลอง/เก็บข้อมูลเอง) · **Secondary = พูดต่อ** (review, ข่าว, สรุป, AI)
- ⚠️ เจอ "งานวิจัยบอกว่า..." → **ไล่กลับไปอ่าน primary** ก่อนเชื่อ; คนกลางบิด/ตัดบริบทบ่อย
- AI สรุปมา = secondary ชั้นที่อาจ hallucinate ต้นทาง → เช็กว่าลิงก์/อ้างอิงมีจริง (ดู `anti-hallucination`)
...
- **เชื่อ vendor whitepaper เป็นหลักฐานกลาง** — มันคือโฆษณา หา primary/แหล่งอิสระยืนยัน
- **เชื่อตัวแทนบริษัทที่อ้างงานวิจัยแต่ไม่ให้ดูตัวเต็ม** — no full text = ไม่นับ
- **ดูตำแหน่ง/ปริญญา/ความดัง แล้วเชื่อ** — authority ≠ หลักฐาน; ดูเนื้อ + CoI
- **ไม่ไล่กลับ primary** — เชื่อ "ข่าว/สรุป/AI พูดต่อ" ทั้งที่ต้นทางอาจถูกบิด
- **เชื่อ AI summary โดยไม่เช็กว่าแหล่งที่มันอ้างมีจริง** — ดู `anti-hallucination`
...
## กับดัก (Anti-patterns)
- **ยัดทุกอย่างเผื่อไว้** — "ใส่หมดเลยเดี๋ยว AI เลือกเอง" → ของสำคัญถูก noise กลบ ตอบแย่ลง ไม่ใช่ดีขึ้น
- **เชื่อว่าแชตยาว = AI จำได้หมด** — มันลืมต้นแชตเงียบๆ โดยไม่บอก แล้วตอบมั่นใจเท่าเดิม
- **ลากงานทั้งโปรเจกต์ในแชตเดียว** — เริ่มแชตใหม่ตามสเต็ปดีกว่า ฝืนแชตเดียวจนเพี้ยน
- **อัปไฟล์ใหญ่ทั้งเล่มเพื่อถามแค่ 2 หน้า** — ตัด/ชี้หน้าที่ต้องการก่อน แม่นกว่า เร็วกว่า
- **ไล่นับ token เป๊ะแทนที่จะดูพฤติกรรม** — ดู "มันเริ่มลืม/วน/ช้า" ง่ายและจริงกว่าตัวเลขประมาณ
...
**Verdict: ให้แต่ละมุม "พยายามทำให้คำตอบตก" ก่อน แล้วค่อยดูว่ารอดไหม — default คือสงสัย ไม่ใช่ไว้ใจ.**
- สั่งตรงๆ: *"สมมติคำตอบนี้ผิด — แต่ละมุมจงหาเหตุผลที่มันผิดมาให้ได้"*
- ห้ามให้มุมไหนตอบลอยๆ ว่า "ดูโอเค" — ต้องชี้จุดที่ตรวจ + เหตุผล
- มุมที่หาเรื่องค้านไม่ได้จริง ค่อยให้ผ่าน (ไม่ใช่ผ่านเพราะขี้เกียจค้าน)
...
> **กฎข้อ 1:** ตัดสิน **จุดประสงค์ + คนอ่าน ก่อนลงมือเขียนประโยคแรก** เสมอ — บทความให้ความรู้ MT, โพสต์เพจ, อีเมล, บล็อกสายอาชีพ ใช้คนละโครงคนละน้ำเสียง. รู้ว่าเขียนให้ใคร-เพื่ออะไร แล้วโครงกับโทนจะตามมาเอง.
> **กับดักข้อ 1:** ยาว ≠ ดี. ย่อหน้าเกริ่นยืดๆ + ศัพท์หรู = คนเลื่อนผ่าน. ของที่ทำให้งานน่าอ่านจริงคือ **เปิดด้วยของจริง (เคส/ตัวเลข/ตัวอย่าง) + ทุกข้ออ้างมีหลักฐานหนุน + ตัดน้ำทิ้ง** ไม่ใช่ความยาวหรือคำสวย.
